...
<div class="card bg-primary mt-3 text-light">
					...
					<tr>
						...
						<td class="text-center">
							<button type="button" class="btn btn-warning"
								onclick="verif(<?=$row->id_daftar_fungsionaris?>,<?=$id_ukm?>)"><i
									class="ti ti-eye"></i></button>
							<button type="button" class="btn btn-danger"
								onclick="hapus(<?=$row->id_daftar_fungsionaris?>)"><i class="ti ti-trash"></i></button>
						</td>
					</tr>
					<?php endforeach;?>
				</tbody>
	...
	<script>
		...
		function verif(id_daftar_anggota, id_ukm) {
			window.open("<?=base_url('cfungsionaris/verif_fungsionaris_form/')?>" + id_daftar_anggota + '/' + id_ukm,
			'_self');
		}
