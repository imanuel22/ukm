...
<script>
	...
	function edit(id_ukm,id_anggota_ukm){
		window.open("<?=base_url('cfungsionaris/fungsionaris_edit/')?>"+id_ukm+'/'+id_anggota_ukm,'_self');
	};
	function hapus(id_ukm,id_anggota_ukm){
		if (confirm('apakah ingin menghapus data id '+id_anggota_ukm+' ini?')) {
			window.open("<?=base_url('cfungsionaris/delete_fungsionaris/')?>"+id_ukm+'/'+id_anggota_ukm,'_self');
		}
	};
</script>
